...
function renderHelp(): OutputLine[] {
  return [
    ...
    ...HELP_ENTRIES.map(([cmd, desc]) =>
      line(seg(cmd.padEnd(CMD_PAD), "text-green-500"), seg(`— ${desc}`, "text-neutral-400")),
    ),
  ];
}
...
function renderAbout(): OutputLine[] {
  return [
    line(seg(`About ${profile.shortName}`, "text-green-500")),
    blank(),
    ...about
      .split("\n\n")
      .flatMap((para) => [
...
function renderProjects(): OutputLine[] {
  const lines: OutputLine[] = [line(seg("Projects", "text-green-500")), blank()];
  projects.forEach((project, i) => {
    lines.push(line(seg(`${i + 1}. `, "text-neutral-400"), seg(project.name, "text-green-500")));
    ...
    if (project.github) {
      lines.push(indented(seg("GitHub: "), seg(project.github, "text-green-500 underline", project.github)));
    }
    project.links?.forEach((link) =>
      lines.push(indented(seg(`${link.label}: `), seg(link.url, "text-green-500 underline", link.url))),
    );
    lines.push(blank());
  });
...
function renderSkills(): OutputLine[] {
  return [
    line(seg("Skills", "text-green-500")),
    blank(),
    ...Object.entries(skills).map(([category, items]) =>
      line(seg(`${category}: `, "text-green-500"), seg(items.join(", "), "text-neutral-300")),
    ),
  ];
}

function renderExperience(): OutputLine[] {
  const lines: OutputLine[] = [line(seg("Experience", "text-green-500")), blank()];
  experience.forEach((job) => {
    lines.push(line(seg(job.role, "text-green-500"), seg(` · ${job.company}`, "text-neutral-400")));
    lines.push(indented(seg(`${job.location} · ${job.dates}`, "text-neutral-500")));
    lines.push(
      indented(
        ...job.tools.flatMap((tool, i) => [
          seg(i === 0 ? "" : " ", "text-neutral-600"),
          seg(`[${tool}]`, "text-green-500"),
        ]),
      ),
    );
...
function renderContact(): OutputLine[] {
  return [
    line(seg("Contact", "text-green-500")),
    blank(),
    line(seg("Email: "), seg(profile.email, "text-green-500 underline", `mailto:${profile.email}`)),
    line(seg("GitHub: "), seg(profile.github, "text-green-500 underline", profile.github)),
    line(seg("LinkedIn: "), seg(profile.linkedin, "text-green-500 underline", profile.linkedin)),
    line(seg("Resume: "), seg("Parks_RPK_Resume.pdf", "text-green-500 underline", profile.resume)),
...
function renderEducation(): OutputLine[] {
  return [
    line(seg("Education", "text-green-500")),
    blank(),
    line(seg(education.school, "text-green-500")),
...
function renderCertifications(): OutputLine[] {
  return [
    line(seg("Certifications", "text-green-500")),
    blank(),
...
function renderLeadership(): OutputLine[] {
  const lines: OutputLine[] = [line(seg("🤝 Leadership & Community", "text-green-500")), blank()];
  leadership.forEach((entry) => {
    lines.push(line(seg(entry.role, "text-green-500"), seg(` · ${entry.org}`, "text-neutral-400")));
